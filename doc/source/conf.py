# -*- coding: utf-8 -*-
#
# PyZgoubi documentation build configuration file, created by
...
# some hacky stuff to make "./setup.py build_doc" work nicely
## make pyzgoubi importable
#install pyzoubi to temp folder
import tempfile, subprocess, os
cwd = os.getcwd()
if not cwd.endswith("doc/source"):
	print "Expected to be in doc/source. actually in ", cwd
else:
	# poke api.rst, so that it always gets rebuilt
	subprocess.Popen(["touch", "api.rst"]).wait()
	pyzgoubi_root_dir = os.path.join(cwd, "..", "..")

	install_dir = tempfile.mkdtemp(prefix='/tmp/pyzgoubi_test_inst_')
	print "installing to", install_dir, "to enable auto api generation"
	print "you can delete this directory afterwards"
#install_res = os.system("python setup.py install --prefix=%s"%install_dir)
#subprocess.Popen(["./setup.py", "clean", "--all"])
	clean_proc = subprocess.Popen(["./setup.py", "clean", "--all"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=pyzgoubi_root_dir)
	clean_proc.wait()
...
latex_documents = [
  ('index', 'PyZgoubi.tex', u'PyZgoubi Documentation',
   u'Sam Tygier', 'manual'),
]
